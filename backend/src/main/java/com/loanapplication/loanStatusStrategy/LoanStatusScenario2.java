...
public class LoanStatusScenario2 implements LoanStatusScenario {
    public LoanApplication createLoanApplication(Client client){
        log.warn("applied loan application scenario is: 2");
        BigDecimal loanAmount = new BigDecimal(10000);

        if( client.getDeposit().compareTo(new BigDecimal(0)) == 1 ){ //deposit > 0
            loanAmount = loanAmount.add(client.getDeposit().multiply(new BigDecimal(0.1))).setScale(2, RoundingMode.HALF_UP); // loanAmount = 10000 + 0.1 * deposit
        }
