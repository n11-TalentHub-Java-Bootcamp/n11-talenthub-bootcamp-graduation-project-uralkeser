...
public class LoanStatusScenario5 implements LoanStatusScenario {
    public LoanApplication createLoanApplication(Client client){
        log.warn("applied loan application scenario is: 5");
        ...
        if( client.getDeposit().compareTo(new BigDecimal(0)) == 1 ){ //deposit > 0
            loanAmount = loanAmount.add(client.getDeposit().multiply(new BigDecimal(0.5))).setScale(2, RoundingMode.HALF_UP); // loanAmount = income * 4 + 0.5 * deposit
        }
